...
private:
    ...
    int level;
public:
	explicit Header(int _level) {text = Text(); level = _level;};
	explicit Header(int _level, Text _text){level = _level; text = Text(_text); text.space_cleaner();};
	explicit Header(int _level, const wchar_t *added) :Header(_level) { text+=added; text.space_cleaner();};
