...
using namespace std;
        ...
        Text::Text(const wchar_t* othr_str, unsigned long othr_len) {
            str = new wchar_t[othr_len+1];
            len = othr_len;
            for (unsigned int i=0; i<othr_len; i++)
                str[i] = othr_str[i];
            str[len++] = L'\0';
        };
        ...
        Text Text::left_space_cleaner(int flag) { //remove only first spaces & return the value
            int ind;
            for (ind=0; (str[ind] == L' ' || str[ind] == L'\t'); ind++);
            int _len = len-ind;
            wchar_t* newstr = new wchar_t[_len];
        ...
        void Text::slice(unsigned long start, unsigned long end) {    // I love python 
            Text buf(*this);
            if (end>len) end=len;
