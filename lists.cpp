...
void Some_List::addText(const wchar_t *added) {txt+=added;}
unsigned long int Some_List::count_symbols() const {return txt.length();}
...
int Some_List::print(std::wostream& os, const Cfg_Attributes& attr) const {
    ...
    if (tabchik + tochkalen + txtlen > attr.terminal_w) {
        if (tabchik + tochkalen + 5 > attr.terminal_w) {
            wcerr << endl;
            wcerr << "Too low terminal width! Can't show text cause it\'s lower \nthan marker + tab lenghts with 5 symbols!\n";
            wcerr << "Teminal width: " << attr.terminal_w << endl << "Marker + tabulation lenght: " << tabchik + tochkalen + 5<< endl;
            return -1;
        }
        else {
            os << Text(L' ', tabchik);
